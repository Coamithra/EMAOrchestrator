...
### Block Persistence Service (`src/main/block-persistence-service.ts`)

Persists CLI event payloads to JSONL files so agent terminal output survives app restarts. Stateless exported functions (same pattern as logging-service). Stored at `app.getPath('userData')/blocks/<agentId>.jsonl`.

- **`appendBlockEvent(agentId, payload)`** — Appends a `CliEventPayload` as a JSON line. Only persists renderable event types (`stream:text`, `step:banner`, `tool:start`, `tool:summary`, `tool:result`, `approval:status`, `session:result`, `error`, `assistant:message`, `orchestrator:inject`). Skips `tool:activity` (high-frequency, low-value for replay) and non-renderable events (`state:changed`, `permission:request`, etc.). Fire-and-forget safe.
- **`readBlockEvents(agentId)`** — Reads and parses all persisted events. Returns empty array on missing/unreadable file.
- **`removeBlockEvents(agentId)`** — Deletes the events file. Called on agent dismiss/destroy and stale agent cleanup.
- **Integration:** The orchestration loop calls `appendBlockEvent()` alongside every `webContents.send('cli:event', ...)` — in `wireDriverToRenderer()`, `emitStepBanner()`, `emitOrchestratorInject()`, and `sendDirectPrompt()`. On app startup, the renderer calls `readBlockEvents()` via IPC for each restored agent, then replays events through `message-stream-service.replayEvents()` to rebuild blocks.
- **IPC channel:** `blocks:getEvents` — renderer fetches persisted events via `window.api.getBlockEvents(agentId)`.
